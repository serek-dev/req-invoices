...
/** @covers \App\Modules\Invoices\Application\InvoiceRejectedHandler */
final class InvoiceRejectedHandlerTest extends TestCase
{
    public function testConstructor(): void
    {
        $sut = new InvoiceRejectedHandler(
            $this->createMock(InvoiceRepositoryInterface::class)
        );
        $this->assertInstanceOf(InvoiceRejectedHandler::class, $sut);
    }

    public function testHandleNonExistingInvoiceThrowsError(): void
    {
        // Given I have an event with non-existing uuid
        $event = $this->createMock(InvoiceRejectedInterface::class);
    ...
    public function testHandleOnInvalidStatusThrowsError(): void
    {
        ...
        // When I handle it
        $sut = new InvoiceRejectedHandler($repository);
        $sut->handle($event);
    }
